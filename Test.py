...
def main():
    ...
    # Ajouter un sélecteur de plage d'années
    st.sidebar.subheader("Sélectionnez une plage d'années")
    years = st.sidebar.slider("",  min_value=int(df_selected_region['year'].min()), max_value=int(df_selected_region['year'].max()), value=(int(df_selected_region['year'].min()), int(df_selected_region['year'].max())))
    df_selected_region = df_selected_region.loc[(df_selected_region['year'] >= years[0]) & (df_selected_region['year'] <= years[1])]
